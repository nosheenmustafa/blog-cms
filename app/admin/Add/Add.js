...
const Add = () => {
  ...
  return (
    ...
    >
      ...
      {errors.author && (
        <span className="font-bold text-red-600">{errors.author.message}</span>
      )}
      <TextField
        label="Description"
      ...
      <input
        ...
        {...register("file", {
          ...
          validate: {
            ...
            fileSize: (files) =>
              files[0]?.size < 5 * 1024 * 1024 || "File size should be < 5MB",
          },
        })}
      />
      {errors.file && (
        <span className="font-bold text-red-600">{errors.file.message}</span>
      )}
